...
const AddMatchModal = ({ open, onClose, onConfirmClose, topicId }) => {
  ...
  const handleSubmit = async () => {
    if (state.options.length === 0) {
    ...
    } else if (botData.bot_speed_to <= botData.bot_speed_from) {
      toast.error("Entry fee must be smaller than winning expected speed.");
    } else if (!logo) {
      ...
      await axiosInstanceAuth
        .post(`/admin/default-match-fees-rewards/add`, formData)
        .then((res) => {
          if (res.data.status) {
        ...
        <Box sx={modalStyle}>
          ...
          <MDBox mx={2} p={1} textAlign="center">
            <MDTypography variant="h4" fontWeight="medium">
              Add Default Match Fees & Rewards
            </MDTypography>
          </MDBox>
